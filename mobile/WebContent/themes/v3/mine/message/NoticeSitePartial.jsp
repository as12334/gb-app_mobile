    ...
    <c:when test="${fn:length(command.result) gt 0}">
        <c:forEach items="${command.result}" var="s">
            <li class="${s.receiveStatus=='12'?'':'no-read'}">
                <div class="i"><a data-rel='{"target":"siteCheck","opType":"function"}' class="${s.receiveStatus=='12'?'gb-radio':'gb-unread-radio'}" value="${s.id}"
                                  name="site1_check"></a>
                    <div class="ct"
                         data-rel='{"target":"${root}/message/announcementDetail.html?searchId=${command.getSearchId(s.id)}","opType":"href"}'>
                        <p>${s.shortTitle50}</p>
                    </div>
