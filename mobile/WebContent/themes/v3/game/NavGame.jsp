            ...
            <div class="mui-scroll">
                <ul class="mui-list-unstyled mui-clearfix mui-bar-tab">
                    <c:forEach var="i" items="${apiTypeRelations}" varStatus="vs">
                        <li><a data-rel='{"target":"changeNavGame","apiTypeId":"${i.apiTypeId}","apiId":"${i.apiId}"}' class="mui-tab-item api-icon-${i.apiTypeId}-${i.apiId} ${vs.index==0?'mui-active':''}">${i.apiName}</a></li>
                    </c:forEach>
                </ul>
            </div>
        </div>
