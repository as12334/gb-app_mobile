        ...
        <c:when test="${fn:length(command.result)>0}">
            ...
            <c:forEach items="${command.result}" var="s">
                <tr data-rel='{"target":"${root}/fund/record/details.html?searchId=${command.getSearchId(s.id)}","opType":"href"}'>
                    <td><span class="text-gray">${soulFn:formatDateTz(s.createTime, DateFormat.DAY,timeZone)}</span>
                    </td>
                    <td>
                        ...
                        <c:if test="${_describe['bitAmount']>0}">
                            ...
                            <span class="text-green2">${empty digiccySymbol?'Ƀ':digiccySymbol}<fmt:formatNumber
                                    value="${_describe['bitAmount']}" pattern="#.########"/></span>
                        </c:if>
                    </td>
                    <td>${dicts.common.transaction_type[s.transactionType]}</td>
                    <td>
                        <c:choose>
                            <c:when test="${s.status=='success'}"><span
                                    class="text-green2">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='failure'}"><span
                                    class="text-red">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='pending_pay'}"><span
                                    class="text-orange">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='pending'}"><span
                                    class="text-blue">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='exchange'}"><span
                                    class="text-blue">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='over_time'}"><span
                                    class="text-red">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='process'}"><span
                                    class="text-blue">${dicts.common.status[s.status]}</span></c:when>
                            <c:when test="${s.status=='reject'}"><span
                                    class="text-red">${dicts.common.status[s.status]}</span></c:when>
